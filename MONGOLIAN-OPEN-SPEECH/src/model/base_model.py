...
class BaseASRModel(torch.nn.Module):
    ...
    def recognize(
        ...
        for i in range(1, maxlen + 1):
            ...
            # logp: (B*N, vocab)
            logp, cache = self.c_decoder.forward_one_step(
...
def init_asr_model(model_init_conf):
    encoder_type = model_init_conf['encoder_type']
    decoder_type = model_init_conf['decoder_type']
    model_conf = model_init_conf['model_conf']
    encoder_conf = model_init_conf['encoder_conf']
    decoder_conf = model_init_conf['decoder_conf']

    if model_init_conf['cmvn_file'] is not None:
        mean, istd = load_cmvn(model_init_conf['cmvn_file'], model_init_conf['raw_wav'])
        global_cmvn = GlobalCMVN(
            torch.from_numpy(mean).float(),
            torch.from_numpy(istd).float())
    else:
        print('global_cmvn is None')
        global_cmvn = None

    input_dim = model_init_conf['input_dim']
    vocab_size = model_init_conf['output_dim']
    ...
    if decoder_type == 'transformer':
        ...
        assert 0.0 < model_conf['reverse_weight'] < 1.0
    ...
    if model_init_conf['pretrain']:
        pretrained_dict = torch.load(model_init_conf['pretrain_path'])
        model_dict = encoder.state_dict()
        # tmp = {}
        # for k, v in pretrained_dict.items() :
        #     if  k[:8] == 'encoder.' and k[8:] in model_dict:
        #         tmp[k[8:]] = v
        #
        # d = [y for y in (list(tmp.keys()) + list(model_dict.keys())) if y not in list(tmp.keys()) ]

        pretrained_dict = {k[8:]: v for k, v in pretrained_dict.items() if k[:8] == 'encoder.' and k[8:] in model_dict}
        model_dict.update(pretrained_dict)
        encoder.load_state_dict(model_dict)
        # for p in encoder.parameters():
        #     p.requires_grad = False

        pretrained_dict = torch.load("/data01/WuYiHao/output/TVQA_训练/pre_finetune/0.pt")
        model_dict = decoder.state_dict()
        pretrained_dict = {k[8:]: v for k, v in pretrained_dict.items() if k[:8] == 'decoder.' and k[8:] in model_dict}
        model_dict.update(pretrained_dict)
        ...
        # pretrained_dict = torch.load("/data01/WuYiHao/output/TVQA_训练/pre_finetune/0.pt")
        # model_dict = ctc.state_dict()
        # pretrained_dict = {k[8:]: v for k, v in pretrained_dict.items() if k[:8] == 'ctc.' and k[8:] in model_dict}
        # model_dict.update(pretrained_dict)
        # state_dict = ctc.load_state_dict(model_dict)


    model = model_init_conf['model'](
