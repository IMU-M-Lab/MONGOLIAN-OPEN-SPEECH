...
class PreDecoder(torch.nn.Module):
    def __init__(self,
                 vocab_size: int,
                 encoder_output_size: int,
                 dropout_rate: float,
        ...
        self.attn = MultiHeadedAttention(attention_heads, attention_dim, self_attention_dropout_rate)
        self.dropout = torch.nn.Dropout(dropout_rate)
        # self.linear_layer = torch.nn.Linear(attention_dim, seq_length)
        self.norm1 = torch.nn.LayerNorm(attention_dim, eps=1e-12)
        # self.norm2 = torch.nn.LayerNorm(seq_length)
        self.sigmoid = torch.nn.Sigmoid()
        self.conv = torch.nn.Conv2d(1, 1, (kernel, attention_dim), padding=(kernel // 2, 0))
        self.vocab_size = vocab_size

    def forward(self, est_text: torch.Tensor):
        x = self.embed(est_text)
        residual = x
        x = residual + self.dropout(self.attn(x, x, x, None))
        x = self.norm1(x)
        x = self.dropout(self.conv(x.unsqueeze(1)).squeeze(3).squeeze(1))
        x = self.sigmoid(x)
        return torch.round(x * self.vocab_size)
