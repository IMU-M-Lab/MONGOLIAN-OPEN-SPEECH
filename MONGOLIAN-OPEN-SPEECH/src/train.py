...
# from dataset.dataset import AudioDataset, CollateFunc
...
def main(rank, world_size, configs, use_cpu):
    # distributed = args.world_size > 1
    distributed = world_size > 1

    if distributed:
        logging.info('training on multiple gpus, this gpu(rank) {}'.format(rank))
        dist.init_process_group(configs['ddp']['dist_backend'],
                                init_method=configs['ddp']['init_method'],
                                world_size=configs['ddp']['world_size'],
                                rank=rank)
    ...
    train_dataset, train_size = Dataset(configs['data_type'], configs['train_data'], symbol_table,
                                        configs['dataset_conf'], configs.get('bpe_model', None), partition=True)
    ...
    train_data_loader = DataLoader(train_dataset,
                                   ...
                                   prefetch_factor=configs['prefetch'],
                                   )
    ...
    train_batch_num = None if distributed else math.ceil(train_size / batch_size) + 1
    cv_batch_num = math.ceil(cv_size / batch_size) + 1
    executor = Executor()
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        executor.train(model, optimizer, scheduler, train_data_loader, device,
                       writer, configs, scaler, train_batch_num)
        total_loss, num_seen_utts = executor.cv(model, cv_data_loader, device,
                                                configs, cv_batch_num)
        if configs['ddp']['world_size'] > 1:
...
if __name__ == '__main__':
    ...
    if args.symbol_table is not None:
        overrides['dict_dir'] = args.symbol_table
    if args.world_size is not None:
        ddp['world_size'] = args.world_size
    if args.dist_backend is not None:
        ddp['dist_backend'] = args.dist_backend
    if args.init_method is not None:
        ddp['init_method'] = args.init_method
    overrides['ddp'] = ddp

    with open(params_file, 'r') as fin:
        configs = load_hyperpyyaml(fin, overrides)  # overrides 的改动不会保存到yaml里
    try:
        mp.spawn(main,
                 args=(configs['ddp']['world_size'], configs, args.use_cpu),
                 nprocs=configs['ddp']['world_size'],
                 join=True)
